#!/bin/bash
# x-bootstrap-developer.sh - Developer editable install
...
if (install_package); then
    ...
    if command -v gh-repo-inspector >/dev/null 2>&1; then
        echo "Successfully installed gh-repo-inspector (editable)"
    ...
    if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
        if [[ -t 0 ]]; then
            echo ""
            echo "Entering virtual environment (editable). Type 'exit' to leave."
            exec $SHELL
        else
            echo ""
